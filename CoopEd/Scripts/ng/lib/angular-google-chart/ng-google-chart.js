...
(function (document, window) {
    ...
    angular.module('googlechart', [])
        ...
        .directive('googleChart', ['$timeout', '$window', '$rootScope', 'googleChartApiPromise', '$log', function ($timeout, $window, $rootScope, googleChartApiPromise, $log) {
            return {
                restrict: 'A',
                scope: {
                    chart: '=chart',
                        ...
                        if (!draw.triggered && ($scope.chart != undefined)) {
                            ...
                            $timeout(function () {
                                ...
                                if ($scope.chart.data instanceof google.visualization.DataTable)
                                    ...
                                    dataTable = google.visualization.arrayToDataTable($scope.chart.data, false);
                                else
                                ...
                                if ($scope.chartWrapper == null) {
                                    ...
                                    google.visualization.events.addListener($scope.chartWrapper, 'ready', function () {
                                        ...
                                        //console.log($scope.chart.actions);
                                        if ($scope.chart.actions != undefined && $scope.chart.actions.length > 0 && $scope.chart.type != 'Table') {
                                            for (var i = 0; i < $scope.chart.actions.length; i++) {
                                                $scope.chartWrapper.getChart().setAction($scope.chart.actions[i]);
                                            }
                                        }
                                    });
                                    google.visualization.events.addListener($scope.chartWrapper, 'error', function (err) {
                                        if (!!$scope.chartWrapper.getContainerId()) {
                                            google.visualization.errors.removeAll($scope.chartWrapper.getContainerId());
                                            console.log(err);
                                        }
                                    });
